...
const CATS = [
  {
    ...
    tint: "from-orange-500/20 to-rose-500/10",
  },
    ...
    <section className="space-y-5">
      <div className="flex items-end justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Pick a vibe — we'll find the best of it.
          </p>
        </div>
      </div>
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
        {CATS.map(({ value, label, icon: Icon, tint }) => (
          ...
          >
            <div
              className={`absolute inset-0 bg-gradient-to-br ${tint} opacity-60 transition-opacity group-hover:opacity-100`}
            />
